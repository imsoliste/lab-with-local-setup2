...
WITH 
  cbc_test AS (SELECT id FROM tests WHERE name = 'Complete Blood Count (CBC)' LIMIT 1),
  pathkind AS (SELECT id FROM labs WHERE name = 'Pathkind Labs' LIMIT 1)
INSERT INTO lab_test_prices (lab_id, test_id, price, discounted_price, home_collection_fee) 
SELECT 
  (SELECT id FROM pathkind),
